...
public class MenuControlador {
    ...
    // Opciones del menú: 0 = Jugar, 1 = Seleccionar Skin
    private int opcionSeleccionada = 0;
    private final int TOTAL_OPCIONES = 2;

    // Flags para comunicarle a Ventana qué hacer
    private boolean iniciarJuego = false;
    private boolean irASkinSelector = false;

    // Para que la tecla no se repita si la mantenés apretada
    ...
    public void draw(Graphics g) {
        ...
        // Instrucción de nave actual
        g.setFont(new Font("Courier New", Font.PLAIN, 14));
        g.setColor(new Color(100, 200, 100));
        String naveInfo = "Nave actual: " + graficos.Assets.nombreSkinActual();
        int anchoInfo = g.getFontMetrics().stringWidth(naveInfo);
        g.drawString(naveInfo, cx - anchoInfo / 2, 600);
    }

    // Ventana consulta estos métodos cada frame
    public boolean debeIniciarJuego() {
        return iniciarJuego;
    }

    public boolean debeIrASkinSelector() {
        return irASkinSelector;
    }
